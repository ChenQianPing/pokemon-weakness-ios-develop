...
open class Promise<T> {
    ...
    */
    public init(resolvers: (_ fulfill: (T) -> Void, _ reject: (Error) -> Void) throws -> Void) {
    ...
    */
    public convenience init(resolver: @escaping ((T?, NSError?) -> Void) throws -> Void) {
        self.init(sealant: { resolve in
            try resolver { obj, err in
                if let obj = obj {
                    resolve(.fulfilled(obj))
                } else if let err = err {
                    resolve(.rejected(err, ErrorConsumptionToken(err as Error)))
                } else {
    ...
    */
    ...
    public init<T: Error>(_ value: T) { abort() }
    ...
    */
    init(sealant: ((Resolution<T>) -> Void) throws -> Void) {
        var resolve: ((Resolution<T>) -> Void)!
        state = UnsealedState(resolver: &resolve)
        do {
            try sealant(resolve)
     ...
     */
    public typealias PendingPromise = (promise: Promise, fulfill: (T) -> Void, reject: (Error) -> Void)
    ...
    */
    open class func pendingPromise() -> PendingPromise {
        var fulfill: ((T) -> Void)!
    ...
    fileprivate convenience init<U>(when: Promise<U>, body: @escaping (Resolution<U>, (Resolution<T>) -> Void) -> Void) {
    ...
    */
    open func then<U>(on q: DispatchQueue = PMKDefaultDispatchQueue()!, _ body: @escaping (T) throws -> U) -> Promise<U> {
    ...
    /**
     ...
     - SeeAlso: `thenInBackground`
    */
    open func then<U>(on q: DispatchQueue = PMKDefaultDispatchQueue()!, _ body: @escaping (T) throws -> Promise<U>) -> Promise<U> {
        return Promise<U>(when: self) { resolution, resolve in
            switch resolution {
            case .rejected(let error):
                resolve(.rejected((error.0, error.1)))
    ...
    open func then<U>(_ on: DispatchQueue = PMKDefaultDispatchQueue()!, _ body: (T) throws -> Promise<U>?) -> Promise<U> { abort() }
    ...
    /**
     ...
     - SeeAlso: `thenInBackground`
    */
    open func then(on q: DispatchQueue = PMKDefaultDispatchQueue()!, body: @escaping (T) throws -> AnyPromise) -> Promise<AnyObject?> {
    ...
    @available(*, unavailable)
    open func then(_ on: DispatchQueue = PMKDefaultDispatchQueue()!, body: (T) throws -> AnyPromise?) -> Promise<AnyObject?> { abort() }
    ...
    */
     ...
     */
    open func errorOnQueue(on q: DispatchQueue = PMKDefaultDispatchQueue()!, policy: ErrorPolicy = .allErrorsExceptCancellation, _ body: @escaping (Error) -> Void) {

        func consume(_ error: Error, _ token: ErrorConsumptionToken) {
            token.consumed = true
            body(error)
        }
        ...
        pipe { resolution in
            ...
            case (let .rejected(error, token), .allErrorsExceptCancellation):
                contain_zalgo(q) {
                    guard let cancellableError = error as? CancellableErrorType, cancellableError.cancelled else {
                        consume(error, token)
                        return
                    }
                }
    ...
    */
    ...
    open func onError(policy: ErrorPolicy = .allErrorsExceptCancellation, _ body: (Error) -> Void) {
    ...
    */
    open func recover(on q: DispatchQueue = PMKDefaultDispatchQueue()!, _ body: @escaping (Error) throws -> Promise) -> Promise {
    ...
    @available(*, unavailable)
    open func recover(_ on: DispatchQueue = PMKDefaultDispatchQueue()!, _ body: (Error) throws -> Promise?) -> Promise { abort() }

    open func recover(on q: DispatchQueue = PMKDefaultDispatchQueue()!, _ body: @escaping (Error) throws -> T) -> Promise {
        return Promise(when: self) { resolution, resolve in
            switch resolution {
            case .rejected(let error, let token):
                contain_zalgo(q, rejecter: resolve) {
    ...
    */
    open func always(on q: DispatchQueue = PMKDefaultDispatchQueue()!, _ body: @escaping () -> Void) -> Promise {
        return Promise(when: self) { resolution, resolve in
            contain_zalgo(q) {
                body()
                resolve(resolution)
    ...
    open func finally(_ on: DispatchQueue = PMKDefaultDispatchQueue()!, body: () -> Void) -> Promise { abort() }
    ...
    open class func defer_() -> (promise: Promise, fulfill: (T) -> Void, reject: (Error) -> Void) { abort() }

    @available(*, deprecated, renamed: "error")
    open func report(policy: ErrorPolicy = .allErrorsExceptCancellation, _ body: (Error) -> Void) { error(policy: policy, body) }

    @available(*, deprecated, renamed: "always")
    open func ensure(on q: DispatchQueue = PMKDefaultDispatchQueue()!, _ body: @escaping () -> Void) -> Promise { return always(on: q, body) }
}
...
func contain_zalgo(_ q: DispatchQueue, block: @escaping () -> Void) {
    if q === zalgo {
        block()
    } else if q === waldo {
...
func contain_zalgo<T>(_ q: DispatchQueue, rejecter resolve: @escaping (Resolution<T>) -> Void, block: @escaping () throws -> Void) {
    contain_zalgo(q) {
        do {
            try block()
        } catch {
...
*/
public func firstly<T>(_ promise: () throws -> Promise<T>) -> Promise<T> {
    do {
        return try promise()
    } catch {
        return Promise(error: error as! Error)
    }
}
...
*/
public func firstly(_ promise: @escaping () throws -> AnyPromise) -> Promise<AnyObject?> {
    return Promise { resolve in
...
public func firstly<T: Error>(_ promise: () throws -> Promise<T>) -> Promise<T> {
...
extension Promise {
    @available(*, unavailable, message: "T cannot conform to ErrorType")
    public convenience init<T: Error>(resolvers: (_ fulfill: (T) -> Void, _ reject: (Error) -> Void) throws -> Void) { abort() }

    @available(*, unavailable, message: "T cannot conform to ErrorType")
    public convenience init<T: Error>(resolver: ((T?, NSError?) -> Void) throws -> Void) { abort() }

    @available(*, unavailable, message: "T cannot conform to ErrorType")
    public convenience init<T: Error>(resolver: ((T, NSError?) -> Void) throws -> Void) { abort() }

    @available(*, unavailable, message: "T cannot conform to ErrorType")
    public class func pendingPromise<T: Error>() -> (promise: Promise, fulfill: (T) -> Void, reject: (Error) -> Void) { abort() }

    @available (*, unavailable, message: "U cannot conform to ErrorType")
    public func then<U: Error>(_ on: DispatchQueue = PMKDefaultDispatchQueue()!, _ body: (T) throws -> U) -> Promise<U> { abort() }

    @available (*, unavailable, message: "U cannot conform to ErrorType")
    public func then<U: Error>(_ on: DispatchQueue = PMKDefaultDispatchQueue()!, _ body: (T) throws -> Promise<U>) -> Promise<U> { abort() }

    @available(*, unavailable, message: "U cannot conform to ErrorType")
    public func thenInBackground<U: Error>(_ body: (T) throws -> U) -> Promise<U> { abort() }

    @available(*, unavailable, message: "U cannot conform to ErrorType")
    public func thenInBackground<U: Error>(_ body: (T) throws -> Promise<U>) -> Promise<U> { abort() }
}
