...
@objc(AnyPromise) open class AnyPromise: NSObject {
    ...
    */
    ...
    convenience public init(bound: Promise<Bool>) {
        self.init(bound: bound.then(on: zalgo) { NSNumber(value: $0 as Bool) })
    }
    ...
    */
    ...
    @objc init(bridge: ((AnyObject?) -> Void) -> Void) {
        var resolve: ((AnyObject?) -> Void)!
        state = State(resolver: &resolve)
        bridge { result in
    ...
    */
    open func then<T>(on q: DispatchQueue = PMKDefaultDispatchQueue()!, body: @escaping (AnyObject?) throws -> T) -> Promise<T> {
    ...
    /**
     Continue a Promise<T> chain from an AnyPromise.
    */
    open func then(on q: DispatchQueue = PMKDefaultDispatchQueue()!, body: @escaping (AnyObject?) -> AnyPromise) -> Promise<AnyObject?> {
    ...
    /**
     Continue a Promise<T> chain from an AnyPromise.
    */
    open func then<T>(on q: DispatchQueue = PMKDefaultDispatchQueue()!, body: @escaping (AnyObject?) -> Promise<T>) -> Promise<T> {
        return Promise(sealant: { resolve in
            pipe { object in
                if let error = object as? NSError {
                    resolve(.rejected(error, error.token))
